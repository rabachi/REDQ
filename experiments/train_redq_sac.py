...
def redq_sac(env_type, env_name, seed=0, epochs='mbpo', steps_per_epoch=1000,
    ...
    # args_dict = OmegaConf.to_container(locals(), resolve=True, throw_on_missing=True)
    wandb.init(
        project="redq",
        entity="viper_svg",
        config=locals(),
    ...
    if env_type == 'gym':
        ...
        domain, task, *_ = env_name.split("-")
        env_fn = lambda: dmc2gym.make(domain_name=domain, task_name=task, seed=seed)
        env, test_env, bias_eval_env = env_fn(), env_fn(), env_fn()
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            epret = logger.log_tabular('EpRet', with_min_and_max=True)
            logger.log_tabular('EpLen', average_only=True)
            testepret = logger.log_tabular('TestEpRet', with_min_and_max=True)
            logger.log_tabular('TestEpLen', average_only=True)
            q1vals = logger.log_tabular('Q1Vals', with_min_and_max=True)
            ...
            wandb.log({"Epoch": epoch}, step=t)
            wandb.log({"TotalEnvInteracts": t}, step=t)
            wandb.log({"EpRet": epret}, step=t)
            wandb.log({"TestEpRet": testepret}, step=t)
            wandb.log({"Q1Vals": q1vals}, step=t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='walker-walk-v99')
    ...
    redq_sac(args.env_type, args.env, seed=args.seed, epochs=args.epochs,
